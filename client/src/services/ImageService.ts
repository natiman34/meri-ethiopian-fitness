...
export class ImageService {
  ...
  private static readonly FOOD_IMAGE_MAPPINGS: Record<string, string> = {
   ...
   */
  public suggestImage(mealName: string, isEthiopian: boolean = false, mealType?: string): string {
    ...
    if (mealType) {
      const fallbackKey = mealType.toLowerCase() as keyof typeof ImageService.FALLBACK_IMAGES;
   ...
   */
  public getImageSuggestions(mealName: string, isEthiopian: boolean = false): string[] {
   ...
   */
  public async uploadImage(file: File, mealId: string): Promise<string> {
   ...
   */
  public getImageWithFallback(imageUrl: string | undefined, mealName: string, isEthiopian: boolean = false): string {
   ...
   */
  public hasImageMapping(mealName: string): boolean {
    const normalizedName = mealName.toLowerCase().trim();
    return Object.keys(ImageService.FOOD_IMAGE_MAPPINGS).some(keyword =>
   ...
   */
  public generatePlaceholderImage(mealName: string, isEthiopian: boolean = false): string {
   ...
   */
  public getImageWithPlaceholder(imageUrl: string | undefined, mealName: string, isEthiopian: boolean = false): string {
    if (imageUrl && imageUrl.trim()) {
      return imageUrl;
    }
