 ...
 * ```
 */
export const validateImageExists = (url, timeout = 5000) => {
 ...
 * ```
 */
export const preloadExerciseMedia = async (exercises, options = {}) => {
    // Input validation
    if (!Array.isArray(exercises)) {
        console.warn('preloadExerciseMedia: exercises must be an array');
        return 0;
    }
    const { maxAssets = 10, delayBetweenAssets = 100, prioritizeImages = true, onProgress, onError } = options;
    ...
    for (const exercise of exercisesToPreload) {
        ...
        if (exercise.image) {
            if (prioritizeImages) {
                ...
                preloadPromises.push(new Promise(resolve => setTimeout(() => preloadImage(exercise.image).then(resolve), delayBetweenAssets)));
            }
        }
        // Preload GIFs (lower priority)
        if (exercise.gifUrl) {
            preloadPromises.push(new Promise(resolve => setTimeout(() => preloadImage(exercise.gifUrl).then(resolve), prioritizeImages ? 1000 : delayBetweenAssets)));
        }
    }
 ...
 * ```
 */
export const getBestExerciseMedia = (exercise, options = {}) => {
    const { preferLocal = true, preferAnimated = true, fallbackUrl = "/images/placeholders/exercise.jpg" } = options;
    // Input validation
 ...
 */
export const isImageApiSupported = () => {
