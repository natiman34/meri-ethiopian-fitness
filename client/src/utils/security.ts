...
export const validatePassword = (password: string): ValidationResult => {
  ...
  if (password.length >= 16) strength += 1; // extra length bonus

  // Bonus for character variety
  const uniqueChars = new Set(password.toLowerCase()).size;
  if (uniqueChars >= password.length * 0.7) strength += 1; // high character variety
 ...
 * ```
 */
export const validateRegistrationData = (data: RegistrationData): RegistrationValidationResult => {
