...
export const useMobileFeatures = (options = {}) => {
    const { enableSwipe = true, enablePullToRefresh = true, swipeThreshold = 50, pullThreshold = 100 } = options;
    // Device detection
    ...
    const [swipeDirection, setSwipeDirection] = useState({ direction: null, distance: 0 });
    ...
    useEffect(() => {
        ...
        if (!container || (!enableSwipe && !enablePullToRefresh))
            return;
